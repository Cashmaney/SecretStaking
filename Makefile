...
_compile-optimized:
	RUSTFLAGS='-C link-arg=-s' cargo build --release --target wasm32-unknown-unknown --features debug-print --locked
...
start-server: # CTRL+C to stop
	docker run -it --rm \
		...
		-v $$(pwd):/root/code \
		--name secretdev enigmampc/secret-network-sw-dev:cash
...
.PHONY: build-all
build-all: compile-optimized
	cd packages/voting_contract/ && RUSTFLAGS='-C link-arg=-s' cargo build --release --target wasm32-unknown-unknown --locked
	cd packages/voting_contract/ && cp ./target/wasm32-unknown-unknown/release/*.wasm ../../build/
	cd packages/staking_token/ && RUSTFLAGS='-C link-arg=-s' cargo build --release --target wasm32-unknown-unknown --locked
	cd packages/staking_token/ && cp ./target/wasm32-unknown-unknown/release/*.wasm ../../build/
